...
void flip270(struct ImBuf * ibuf)
{
	...
	memcpy(ibuf->rect, tbuf->rect, ibuf->x * ibuf->y * sizeof(ulong));
	x = ibuf->x;
	ibuf->x = ibuf->y;
	ibuf->y = x;
	freeImBuf(tbuf);
...
{
	short x, y, halfx;
	ulong *left, *right, temp, leftadd, rightadd;
...
void shearx(struct ImBuf *ibuf, float dist)
{
	...
	for (y = 0; y < ibuf->y; y++){
		...
		drect += ((long) tdist) * sizeof(ulong);
		w1 = tdist - floorf(tdist);
		tdist += dist;
		if (w1 <= MINSHDIST) {
			memcpy(drect, srect, ibuf->x * sizeof(ulong));
		} else if ( w1 >= 1.0 - MINSHDIST) {
			memcpy(drect + sizeof(ulong), srect, ibuf->x * sizeof(ulong));
		} else {
			w2 = 1.0 - w1;
			la = lb = lg = lr = 0.0;
			for (x = ibuf->x; x > 0; x--){
