...
long * hamx_array = (long *) (hamx_array_char);
...
struct ImBuf *ibuf;
{
	...
	long cy, ch, cs, ly, lh, ls, dy, dh, ds, nh, ny, ns, fh, fy, fs, ty, t, col, type, step, first;
	long i, error, minerror;
	uchar *rect;
...
short dec_and_or_rect(ImBuf * ibuf, uchar * body, short * and_or, long first)
{
	long todo, i;
	long j, step, col;
	ulong *rect;
...
short dec_and_or_short(ImBuf * ibuf, uchar * body, short * and_or, long first)
{
	long todo, i;
	long j, step, col;
	short *rect;
...
struct ImBuf *loadanim(long *iffmem, long flags)
{
	long chunk, totlen, len, *mem, cmaplen = 0;
	ulong *cmap;
	uchar *body = 0;
	struct Adat adat;
	struct ImBuf *ibuf=0;
	extern float rgb_to_bw[4][4];
	static long first_tanx = TRUE;
	static long hamx_rgba = 0;
	...
	case HAMX:
		if (flags & IB_rect){
			if ((flags & IB_rgba) != hamx_rgba) {
				int i;
				ulong * t;
				t = (ulong *) hamx_array_char;
				
				for (i = 0; i < sizeof(hamx_array_char) / sizeof(long) ; i++) {
					t[i] = SWAP_LONG(t[i]);
				}
				
				t = (ulong *) cmap_hamx;
				
				for (i = 0; i < sizeof(cmap_hamx) / sizeof(long) ; i++) {
					t[i] = SWAP_LONG(t[i]);
				}
								
				hamx_rgba = flags & IB_rgba;
			}
