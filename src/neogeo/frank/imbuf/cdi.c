...
void codedyuv(uchar *buf, uchar *enc, long x, long start)
{
	long col, dif, code, ncol;
	long i;
...
long file;
{
	...
	uchar enctab[256];
	long x, y, sizex, halfx, size, start;
	short ok = TRUE;
...
long *iffmem,flags;
{
	long chunk,totlen,len,*mem,*cmap=0,cmapofs,cmaplen;
	uchar *idat=0;
	struct Ihdr ihdr;
	struct ImBuf *ibuf=0;
