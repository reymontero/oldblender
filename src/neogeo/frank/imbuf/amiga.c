...
short encodebodyh(ibuf,file)
struct ImBuf *ibuf;
long file;
{
	uchar *buf, *endbuf, *max;
	long size, line, ok = TRUE;
	ulong **list;
	short skipx,i,y;
...
uchar *body;
{
	long skipbuf,skipbdy,depth,y,offset = 0;
...
struct ImBuf *loadamiga(long *iffmem,long flags)
{
	long chunk,totlen,len,*cmap=0,cmaplen,*mem,ftype=0;
	uchar *body=0;
