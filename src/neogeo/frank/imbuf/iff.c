...
ushort start_iff(ibuf,file)
struct ImBuf *ibuf;
long file;
{
	ulong *point, size, *buf, start;
	
	if ((point=buf=(ulong *)malloc(32768))==0) return FALSE;
	...
	*point++ = 0;
	...
	if (IS_amiga(ibuf)){
		...
		*point++=sizeof(struct BitMapHeader);
		...
		point=(ulong *)((char *)point+sizeof(struct BitMapHeader));	/* pointer alvast verder zetten */
		...
		*point++=sizeof(struct Ihdr);
		...
		point=(ulong *)((char *)point+sizeof(struct Ihdr));	/* pointer alvast verder zetten */
		...
		*point++ = BIG_LONG(sizeof(struct Adat));
		...
		point = (ulong *)((char *)point+sizeof(struct Adat));	/* pointer alvast verder zetten */
...
ushort update_iff(file, code)
long file, code;
{
	long	buf[2], filelen, skip;
	uchar nop;

	if (file<=0) return (FALSE);

	/* lseek: 0=start, 1=current, 2=eind */

	filelen = lseek(file,0L,2)-8;			/* filelengte berekenen */
