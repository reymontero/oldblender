...
long Cspace_err_diff = FALSE;

long cspace_err_diff(long new)
{
	long old;
...
long colcspace(long col,float mat[][4])
{
	float r,g,b;
	short x,y,z;
...
short *cont_1,*cont_2,*cont_3,*add;
{
	...
	if (buf){
		for(y=ibuf->y;y>0;y--){
			cspfill(buf,add,x);
			cspadd(buf,cont_1,rect+1,x);
			cspadd(buf,cont_2,rect+2,x);
			cspadd(buf,cont_3,rect+3,x);
			cspret(buf,rect,x);
			rect += x<<2;
		}
		free(buf);
