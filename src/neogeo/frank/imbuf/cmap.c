...
void losecmapbits(ibuf, coltab)
struct ImBuf *ibuf;
ulong * coltab;
{
	long i,bits;
	ulong col,and1,and2,*rect;
...
short addplanetocube(cube, plane, minx, miny, sizep, addcx, addcy, sizec, col)
short *cube, *plane;
long minx, miny, sizep, addcx, addcy, sizec, col;
{
	short done = FALSE;
	long x, numx, numy, skipc, skipp, temp;
...
short *coldeltatab(uchar *coltab, short mincol, short maxcol, short cbits)
{
	...
	for (i = 1; i <= max ; i++){
		...
		for (j = mincol; j < maxcol; j++){
			...
			g = col[2] - i;
			r = col[3] - i;
			cube = _cube + (addcr * r) + (addcg * g) + (addcb * b);
...
short *deltab,cbits;
{
	...
	short x,y;
	ulong col;
	ulong bbits,gbits,rbits;
	ulong bmask,gmask,rmask;
...
struct ImBuf* ibuf;
{
	...
	coltab = calloc(ibuf->maxcol, sizeof(ulong));
	if (coltab == 0) return(0);
	memcpy(coltab, ibuf->cmap, ibuf->maxcol * sizeof(ulong));
...
struct ImBuf *ibuf;
{
	...
	if (IS_ham(ibuf)){
		...
		mask = malloc(1024 * 2 * sizeof(long));
		...
		for(i = ibuf->x * ibuf->y; i>0; i--){
/*			col = *rect;
			if (col >= 0 && col < ibuf->maxcol) *rect = cmap[*rect];
			rect++;
*/
			*(rect++) = cmap[*rect];
		}
