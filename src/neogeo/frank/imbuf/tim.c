...
void apply_tim_cmap(ulong * data, ulong count) {
	long col, a, b, g, r, cur;
...
void to_tim_cmap(ulong * data, ulong count) {
	long col, a, b, g, r, cur;
...
struct ImBuf *loadtim(ushort * mem, long flags)
{
	TARGA tga;
	struct ImBuf * ibuf;
	long count, size, type, clut, x, y, col, depth;
	ulong * rect;
	uchar * cmem;
...
int write_tim_short_data(long file, ulong * data, ulong count)
{
	long i, ok = TRUE;
	short * _mem, * mem;
...
short savetim(struct ImBuf * ibuf, long file, long flags)
{
	long buf[32], size, ok = TRUE;
