...
typedef struct {
	CVECTOR c0;
	u_char u0, v0;	short clut;
	u_char u1, v1;	short tpage;
	u_char u2, v2;	short tw_nr;
...
typedef struct pDFace {
	int dist;
	ushort ocx, ocy, ocz, no_nr;
	ushort index, rt;					/* index: (vertex-index) in aantal words vanaf packetdata */
	char proj, mat_nr, quad, edcode;	/* quad == TRUE: vierkant */
} pDFace;
