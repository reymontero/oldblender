...
void insert_hashedge(struct EditEdge *eed)
{
...
void remove_hashedge(struct EditEdge *eed)
{
...
/* tijd= clock(); */
		...
		if(vv->vlak) {
			...
			for(a=0;a<vv->vlak;a++)	{
				...
				evl= addvlaklist(eve1,eve2,eve3,(adrvl->ec & 31));
				adrvl++;
			}
		}
...
void load_ebasedata()
{
	...
	if(G.ebase->soort==1) {
		...
		while(eve) {
			...
			eve->vn= (struct EditVert *)a++;  /* teller */
			eve= eve->next;
			adrve++;
		}
		...
		evl= (struct EditVlak *)G.edvl.first;
		while(evl) {
			adrvl->v1= (int) evl->v1->vn;
			adrvl->v2= (int) evl->v2->vn;
			adrvl->v3= (int) evl->v3->vn;
			adrvl->f= 0;
			adrvl->ec= evl->f1;
			adrvl++;
			evl= evl->next;
		...
		while(eed) {
			if( eed->f==0 ) {
				adrvl->v1= (int) eed->v1->vn;	   /* v1==v2 wordt door initrender afgevangen */
				adrvl->v2= (int) eed->v1->vn;
				adrvl->v3= (int) eed->v2->vn;
				adrvl->ec= 0;
				adrvl++;
			}
			eed= eed->next;
		...
		if(ob->rt3) {
			...
			while(eve) {
				if(eve->vnorm) {
					cp= (char *)&(eve->vnorm);
					adrve->n[0]= adrve->n[1]= adrve->n[2]= 0;
					*( (char *)adrve->n)= cp[0];
					*( (char *)(adrve->n+1))= cp[1];
					*( (char *)(adrve->n+2))= cp[2];
				}
			...
			/* OPNIEUW: puntnormaal omklap-vlaggen voor bij shade*/
			adrve= (struct VertOb *)(vv+1);
			for(a=0; a<vv->vlak; a++) {
	...
	*/
	...
	while(eed) {
		...
		if( (eed->f==1 || eed->f==2) ) {
			...
			v2= eed->v2->vn;
			if(toggle & 1) {
				addvlaklist(eed->v1,v1,v2,G.colact-1);
				addvlaklist(eed->v1,v2,eed->v2,G.colact-1);
			}
			else {
				addvlaklist(eed->v2,eed->v1,v1,G.colact-1);
				addvlaklist(eed->v2,v1,v2,G.colact-1);
			}
		}
	...
	while(evl) {
		...
		if(evl->f & 1) {
			...
			v2= evl->v2->vn;
			v3= evl->v3->vn;
			evln= addvlaklist(v1,v2,v3,evl->f1);
...
float limit;
{
	...
	struct EditVert *eve, *v1, *v2, *v3, *nextve, *vn;
	...
	while(evl) {
		...
		if(evl->f==1) {
			...
			else if(evl->v2==evl->v3) test++;
			else if(evl->v3==evl->v1) test++;
	...
	if(aantal) {
		...
		while(evl) {
			if(evl->f & 1) {
				vsb->x= (int) MIN3( (int)evl->v1, (int)evl->v2, (int)evl->v3);
				vsb->evl= evl;
				vsb++;
			}
			evl= evl->next;
...
short flag;
{
	...
	parentbase(G.ebase, mat, omat);
	Mat4MulMat4(pmat, pmat, G.viewmat);
	...
	while(eve) {
		if(eve->f & flag) {
			sb->x= pmat[0][0]*eve->co[0]+pmat[0][1]*eve->co[1]+pmat[0][2]*eve->co[2]+pmat[0][3];
			sb->v1= eve;
			sb++;
		}
		eve= eve->next;
...
float rad;
{
	...
	while(evl) {
		if( (evl->v1->f & flag) || (evl->v2->f & flag) || (evl->v2->f & flag) ) {
			...
			if(test) {
				if((test & 3)==3) addvlaklist(evl->v2,e2->vn,e1->vn,evl->f1);
				if((test & 6)==6) addvlaklist(evl->v3,e3->vn,e2->vn,evl->f1);
				if((test & 5)==5) addvlaklist(evl->v1,e1->vn,e3->vn,evl->f1);
				if(test==7) {  /* vier nieuwe vlakken, oude vernieuwt */
					...
					addvlaklist(e1->vn,e2->vn,evl->v3,evl->f1);
					evl->v2= e1->vn;
				}
				else if(test==6) {
					addvlaklist(e2->vn,e3->vn,evl->v1,evl->f1);
					evl->v3= e2->vn;
				}
				else if(test==5) {
					addvlaklist(e3->vn,e1->vn,evl->v2,evl->f1);
					evl->v1= e3->vn;
				}
				else if(test==1) {
					addvlaklist(e1->vn,evl->v2,evl->v3,evl->f1);
					evl->v2= e1->vn;
					addedgelist(evl->v1,evl->v2);
				}
				else if(test==2) {
					addvlaklist(e2->vn,evl->v3,evl->v1,evl->f1);
					evl->v3= e2->vn;
					addedgelist(evl->v2,evl->v3);
				}
				else if(test==4) {
					addvlaklist(e3->vn,evl->v1,evl->v2,evl->f1);
					evl->v1= e3->vn;
...
void vertroteer()
{
	...
	long  a, teller=0;
	short xc=0,yc=0,val,mval[2],toets;
...
void vertbend()
{
	...
	long  a, *rt=0, teller=0;
	short ys, yc, yco=0, val, mval[2], toets;
	char s[40];
	...
	while(TRUE) {
		...
		if(yc!=yco) {
			yco= yc;
			omtrekfac= startomtrekfac+0.008*(yc-ys);
...
void vertsizing()
{
	...
	long a, teller=0,teller2,a1;
	short val,mval[2],xm,ym,xn,yn,xc=0,yc=0,toets;
	short midtog=0,proj;
	Device mdev[2];
	char s[20];
...
void vertices_to_sphere()
{
	struct EditVert *eve;
	struct ObData *ob;
	float len, vec[3], cent[3], imat[3][3], tmat[4][4], bmat[3][3];
	float VecLenf();
	int tot;
	...
	/* centrum */
	cent[0]= view0.muis[0]-tmat[3][0];
	cent[1]= view0.muis[1]-tmat[3][1];
	cent[2]= view0.muis[2]-tmat[3][2];
	Mat3MulVecfl(imat, cent);
	...
	while(eve) {
		if(eve->f & 1) {
			...
			eve->co[0]= cent[0]+vec[0]*len;
			eve->co[1]= cent[1]+vec[1]*len;
			eve->co[2]= cent[2]+vec[2]*len;
...
void leesCurryEdit(char *name)
{
	struct Base *base;
	struct ObData *ob;
	struct VV *vv;
	struct EditVert *v1, *v2, *v3;
	struct EditVert *v1o=0, *v2o=0, *v3o=0;
	struct ColBlck *col;
	struct VlakOb *adrvl;
	struct VertOb *adrve;
	float vec[3];
	...
	while(fd<end-17) {
		if(v1o && fd[0]==fd[-18] && fd[1]==fd[-17] && fd[2]==fd[-16]) {
			v1= v1o;
		}
		else if(v2o && fd[0]==fd[-12] && fd[1]==fd[-11] && fd[2]==fd[-10]) {
			v1= v2o;
		}
		else if(v3o && fd[0]==fd[-6] && fd[1]==fd[-5] && fd[2]==fd[-4]) {
			v1= v3o;
		}
		else {
			svec[0]= 2*((fd[0]<<7)-16384);
			svec[1]= 2*((fd[1]<<7)-16384);
			svec[2]= 2*((fd[2]<<7)-16384);
			VECCOPY(vec, svec);
			v1= addvertlist(vec);
		}
		
		if(v1o && fd[6]==fd[-18] && fd[7]==fd[-17] && fd[8]==fd[-16]) {
			v2= v1o;
		}
		else if(v2o && fd[6]==fd[-12] && fd[7]==fd[-11] && fd[8]==fd[-10]) {
			v2= v2o;
		}
		else if(v3o && fd[6]==fd[-6] && fd[7]==fd[-5] && fd[8]==fd[-4]) {
			v2= v3o;
		}
		else {
			svec[0]= 2*((fd[6]<<7)-16384);
			svec[1]= 2*((fd[7]<<7)-16384);
			svec[2]= 2*((fd[8]<<7)-16384);
			VECCOPY(vec, svec);
			v2= addvertlist(vec);
		}
		
		if(v1o && fd[12]==fd[-18] && fd[13]==fd[-17] && fd[14]==fd[-16]) {
			v3= v1o;
		}
		else if(v2o && fd[12]==fd[-12] && fd[13]==fd[-11] && fd[14]==fd[-10]) {
			v3= v2o;
		}
		else if(v3o && fd[12]==fd[-6] && fd[13]==fd[-5] && fd[14]==fd[-4]) {
			v3= v3o;
		}
		else {
			svec[0]= 2*((fd[12]<<7)-16384);
			svec[1]= 2*((fd[13]<<7)-16384);
			svec[2]= 2*((fd[14]<<7)-16384);
			VECCOPY(vec, svec);
			v3= addvertlist(vec);
		}
		fd+= 18;
