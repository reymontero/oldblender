...
float beztol = 100.0;
unsigned char *subrs[MAXSUBRS];
unsigned int sublen[MAXSUBRS];
unsigned char *chars[MAXCHARS];
unsigned int charlen[MAXCHARS];
char *charname[MAXCHARS];
...
objfnt * loadpostcriptfont(char * filename)
{
	...
	for(i=0; i<MAXCHARS; i++) {
		...
		namelen = strlen(tok);
		charname[i] = (char *)mallocN(namelen+1, "charname");
...
int x, y;
{
	...
	dx = nextx-curx;
	dy = nexty-cury;
	if (savesplines) spline_line((float)curx,(float)cury,(float)nextx,(float)nexty);
	else drawline((float)curx,(float)cury,(float)nextx,(float)nexty,dx,dy,dx,dy);
	curx = nextx;
	cury = nexty;
}
...
drawline(x0,y0,x1,y1,dx0,dy0,dx1,dy1)
float x0,y0,x1,y1;
...
int savesplines;
{
	...
	if(savesplines)
		fnt = newobjfnt(SP_TYPE,32,32+NASCII-1,1000);
	else
		fnt = newobjfnt(PO_TYPE,32,32+NASCII-1,1000);
