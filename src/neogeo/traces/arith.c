...
/*
	...
	Mat3MulSerie(answ, m1, m2, m3....)
	
	Mat4CpyMat4(mat1,mat2)				    (mat1=mat2)
	Mat3CpyMat4(mat1,mat2)
	Mat4CpyMat3(mat1,mat2)				    (geen clear)
...
short	FloatCompare(v1,v2,limit)
...
long	VecLen(v1,v2)					    (afstand tussen twee punten)
float	VecLenf(v1,v2)
	VecAddf(v, v1,v2)
	VecSubf(v, v1,v2)
	VecMidf(v, v1,v2)
...
float mat[][4];
{
	...
	mat[3][2] = t;
								...
								/* IS NIET GOED!!! */
void Mat4Inverto(to,from)	    /* uit insane/libwin/matrix.c van Haeberli */ 
float to[][4], from[][4];		/* LET OP: ALLEEN MET TRANSPOSE MATRIXEN */
{
	double wtemp[4][8];			/* doubles voor betere resolutie zeer kleine getallen */
	double m0,m1,m2,m3,s;
	double *r0,*r1,*r2,*r3, *rtemp;
...
#define SWAP(a,b,t) t = a;a = b;b = t
	...
	for(i = 0; i < 4; i++) {
		...
		if (maxj != i) {
			for( k = 0; k < 4; k++) {
				SWAP(tempmat[i][k],tempmat[maxj][k],temp);
				SWAP(inverse[i][k],inverse[maxj][k],temp);
			}
		}
...
void Mat4MulFloat3(m,f)		/* alleen de scale component */
float m[][4];
float f;
{
	long i,j;
...
void Mat3ToQuatwerktniet(wmat, q)		/* uit Adv.An. & Rend.Tech. pag 363 */
float wmat[][3], *q;
{
	void Mat3Ortho();
	double tr, s;
	...
	if(tr>0.0) {
		...
		q[i+1]= s*0.5;
		s= 0.5/s;
		q[0]= (mat[j][k]-mat[k][j])*s;
		q[j+1]= (mat[i][j]-mat[j][i])*s;
		q[k+1]= (mat[i][k]-mat[k][i])*s;
	}
}
...
void Mat3ToQuatwerkt(mat, q)
float mat[][3], *q;
{
	float v1[3], v2[3], v3[3];

	/* methode: met triatoquat (anim.c) , niet gebruiken omdat arith.c zelfstandig moet bliven*/
	v1[0]=v1[1]=v1[2]= 0.0;
	/* VECCOPY(v2, mat[0]); */
	/* VECCOPY(v3, mat[1]); */
...
{
	static float q1[4];
	float up[3], q2[4], nor[3], *fp, mat[3][3], hoek, si, co, x2, y2, z2, len1, Normalise();
...
i_lookat(vx, vy, vz, px, py, pz, twist, mat)
...
void CalcCent4f(cent, v1, v2, v3, v4)
float *cent, *v1, *v2, *v3, *v4;
{
...
float DistVL2Dfl_oud(v1,v2,v3)   /* met formule van Hesse :GEEN LIJNSTUK! */
float *v1,*v2,*v3;
{
	float a[2],fi,deler;
	...
	a[0]= v2[1]-v3[1];
	a[1]= v3[0]-v2[0];
	fi= a[0]*v2[0]+a[1]*v2[1];
	deler= fsqrt(a[0]*a[0]+a[1]*a[1]);
	if(deler== 0.0) return 0;

	return fabs(v1[0]*a[0]+v1[1]*a[1] -fi)/deler;

}

							 /* afstand v1 tot lijn v2-v3 */
float DistVL2Dfl(v1,v2,v3)   /* met formule van Hesse :GEEN LIJNSTUK! */
float *v1,*v2,*v3;
{
	float a[2],deler;
...
float PdistVL2Dfl(v1, v2, v3)  /* PointDist: WEL LIJNSTUK */
float *v1,*v2,*v3;
{
...
float AreaQ3Dfl(v1, v2, v3, v4)  /* only convex Quadrilaterals */
float *v1, *v2, *v3, *v4;
{
...
float AreaPoly3Dfl(nr, verts, normal)
long nr;
float *verts, *normal;
{
