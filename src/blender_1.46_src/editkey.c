...
void insert_meshkey(Mesh *me)
{
	...
	curpos= system_time(0, 0, (float)CFRA, 0.0);
...
void showkeypos(Key *key, KeyBlock *kb)
{
	...
	if(key == give_current_key(ob)) {
		...
		if(ob->type==OB_MESH) {
			...
			cp_key(0, me->totvert, me->totvert, (char *)me->mvert->co, me->key, kb);
		}
		else if(ob->type==OB_LATTICE) {
			lt= ob->data;
			tot= lt->pntsu*lt->pntsv*lt->pntsw;
			
			cp_key(0, tot, tot, (char *)lt->def->vec, lt->key, kb);
