...
MVid		movie, image; 
DMparams	*movie_params, *image_params;
int			compr_params[64];
int			index, qualindex, qualnow, mv_outx, mv_outy, numfields= 2;
char		*comp_buf;
int			sfra, efra, first = TRUE, maxbufsize;
int			ntsc = FALSE;
...
int my_Compress(ulong * rect, int *bufsize)
{
	int err = 0;
...
void start_movie()
{
	...
	fd = open(name, O_RDWR | O_CREAT | O_EXCL, 0664);
	if (fd != -1) {
		if (flock(fd, LOCK_EX) == -1) report_flock();
			new_movie(fd);
