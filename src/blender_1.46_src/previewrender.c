...
ulong previewback(int type, int x, int y)
{
	if(type & 2) {
		if(abs(x)>abs(y)) return 0;
		else return 0x01404040;
	}
	else {
		if(abs(x)>abs(y)) return 0x01404040;
		else return 0x01a0a0a0;
	}
}
...
void set_previewrect(int xmin, int ymin, int xmax, int ymax)
{
	...
	pr_facx= ( (float)pr_sizex)/PR_RECTX;
	pr_facy= ( (float)pr_sizey)/PR_RECTY;
}

void display_pr_scanline(ulong *rect, int starty)
{
	ulong *scan;
	int x, y, dx, dy, endy;
	int tx=0, ty;
	...
	for(y=starty; y<= endy; y++) {
		lrectwrite(prerect.xmin, y, prerect.xmax-1, y, pr_scan);
	}
}

void display_pr_scanline_O2(ulong *rect, int starty)
{
	ulong *scan;
	int x, y, dx, dy, endy;
	int tx=0, ty;
	...
	/* tegelijk schrijven in front&backbuffer op O2 gaat fout */
	/* dus scanline nog eens neerzetten, front&backbuf moeten indentiek zijn */
...
void draw_tex_crop(Tex *tex)
{
	...
	if(tex->type==TEX_IMAGE) {
		...
		cpack(0x0);
		sboxi(rct.xmin+1, rct.ymin-1, rct.xmax+1, rct.ymax-1);
		cpack(0xFFFFFF);
		sboxi(rct.xmin, rct.ymin, rct.xmax, rct.ymax);			
	}
...
void previewdraw()
{
	...
	EmbossBox3(PR_XMIN, PR_YMIN, PR_XMAX, PR_YMAX, 1, 0x202020, 0x808080);
...
void lamp_preview_pixel(LampRen *la, int x, int y, char *rect)
{
	...
	col= 255.0*dist*la->r;
	if(col<=0) rect[3]= 0; else if(col>=255) rect[3]= 255; else rect[3]= col;
...
void previewflare(HaloRen *har, ulong *rect)
{
	float ycor;
	ulong *rectot;
...
void shade_preview_pixel(int x, int y, char *rect, int smooth)
{
	...
	if(mat->mode & MA_SHLESS) {
		temp= 255.0*(mat->r);
		if(temp>255) rect[3]= 255; else if(temp<0) rect[3]= 0; else rect[3]= temp;
	...
	if(mat->alpha!=1.0) {
		...
		rect[3]= tracol+ (rect[3]*alpha) ;
		rect[2]= tracol+ (rect[2]*alpha) ;
		rect[1]= tracol+ (rect[1]*alpha) ;

	}
	if(G.machine==IRIS) {	/* dither */
		if(y & 1) {
			if(x & 1);
			else {
				if(rect[3]<248) rect[3]+= 8;
				if(rect[2]<248) rect[2]+= 8;
				if(rect[1]<248) rect[1]+= 8;
			}
		}
		else {
			if(x & 1) {
				if(rect[3]<252) rect[3]+= 4;
				if(rect[2]<252) rect[2]+= 4;
				if(rect[1]<252) rect[1]+= 4;
			}
			else {
				if(rect[3]>4) rect[3]-= 4;
				if(rect[2]>4) rect[2]-= 4;
				if(rect[1]>4) rect[1]-= 4;
			}
		}
	}
}
...
void previewrender()
{
	...
	if(G.buts->mainb==BUTS_MAT) {
		...
		/* imats wissen */
		for(x=0; x<8; x++) {
			if(mat->mtex[x] && mat->mtex[x]->tex) init_render_texture(mat->mtex[x]->tex);
			if(mat->mtex[x] && mat->mtex[x]->object) Mat4One(mat->mtex[x]->object->imat);
		}
		R.vlr= 0;
		R.mat= mat;
		R.matren= mat->ren;
	...
	for(y=starty; y<endy; y++) {
		...
		if(y<endy-2) {
			...
			cpack(0x0);
			ysq= PR_YMIN+1+ ((G.buts->cury+2)*(PR_YMAX-PR_YMIN-4)/(PR_RECTY));
			sboxfi(PR_XMIN+4, ysq, PR_XMAX-4, ysq);
		...
		display_pr_scanline_O2(G.buts->rect, G.buts->cury);
