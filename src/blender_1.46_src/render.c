...
void sky(char *col)
{
	...
	if(R.wrld.skytype & WO_SKYBLEND) {
		...
		col[3]= 255.0*R.wrld.horr;
		col[2]= 255.0*R.wrld.horg;
		col[1]= 255.0*R.wrld.horb;
	}
	col[0]= 1;	/* om verkeerde optimalisatie alphaover van flares te voorkomen */
}

void scanlinesky(char *rect, int y)
{
	void (*alphafunc)();
	extern void addalphaUnderGamma(), addalphaUnder();
	float u, v, fac;
	int dx, x, ofs;
	ulong col=0, *rt;
	short *sp;
	...
	if(R.r.alphamode & R_ALPHAKEY) {
		...
		cp= (char *)&col;
		cp[0]= 1;
		cp[3]= 255.0*R.wrld.horr;
		cp[2]= 255.0*R.wrld.horg;
		cp[1]= 255.0*R.wrld.horb;
		...
		for(x=0; x<R.rectx; x++, rect+= 4) {
			if(rect[0]==0) {
				*( ( ulong *)rect)= col;
				...
				cp1= (char *)rect;
				if( cp[1]==cp1[1] && cp[2]==cp1[2] && cp[3]==cp1[3] ) {
	...
	if((R.wrld.skytype & (WO_SKYBLEND+WO_SKYTEX))==0) {
		for(x=0; x<R.rectx; x++, rect+= 4) {
			if(rect[0]!=255) {
				if(rect[0]==0) *((ulong *)rect)= R.wrld.fastcol;
				else {
...
{
	float t0, t1, t2, t3, a, b, c, disc, haint;
...
void shadehalo(HaloRen *har, char *col, ulong zz, float dist, float xn, float yn, short flarec)
{
	/* in col invullen */
...
ulong calchalo_z(HaloRen *har, ulong zz)
{
...
void scanlinehaloPS(ulong *rectz, int *rectdelta, ulong *rectt, short ys)
{
	HaloRen *har;
	PixStr *ps;
	float dist,xsq,ysq,ster,xn,yn,zn,t;
	ulong a, *rz, *rt, zz, ztot;
	int *rd;
	long accol[4];
...
void scanlinehalo(rectz, rectt, ys)
ulong *rectz, *rectt;
short ys;
{
	HaloRen *har;
...
void shadelamplus()
{
	...
	float lampcol[3], *vn, *view, shadfac, soft;
	extern float testshadowbuf(ShadBuf *, float);
	int a, addcol, c1, c2;
	...
	if(ma->texco) {
		...
		if(ma->texco & TEXCO_REFL) {
			...
			R.ref[0]= (view[0]+i*vn[0]);
			R.ref[1]= (view[1]+i*vn[1]);
			R.ref[2]= (view[2]+i*vn[2]);
			if(R.osatex) {
				if(R.vlr->flag & R_SMOOTH) {
					i= -2*( (vn[0]+O.dxno[0])*(view[0]+O.dxview) +
					    (vn[1]+O.dxno[1])*view[1]+ (vn[2]+O.dxno[2])*view[2] );

					O.dxref[0]= R.ref[0]- ( view[0]+O.dxview+i*(vn[0]+O.dxno[0]));
					O.dxref[1]= R.ref[1]- (view[1]+ i*(vn[1]+O.dxno[1]));
					O.dxref[2]= R.ref[2]- (view[2]+ i*(vn[2]+O.dxno[2]));

					i= -2*( (vn[0]+O.dyno[0])*view[0]+
					    (vn[1]+O.dyno[1])*(view[1]+O.dyview)+ (vn[2]+O.dyno[2])*view[2] );

					O.dyref[0]= R.ref[0]- (view[0]+ i*(vn[0]+O.dyno[0]));
					O.dyref[1]= R.ref[1]- (view[1]+O.dyview+i*(vn[1]+O.dyno[1]));
					O.dyref[2]= R.ref[2]- (view[2]+ i*(vn[2]+O.dyno[2]));
					...
					O.dxref[0]= R.ref[0]- (view[0]+O.dxview+i*vn[0]);
					O.dxref[1]= R.ref[1]- (view[1]+ i*vn[1]);
					O.dxref[2]= R.ref[2]- (view[2]+ i*vn[2]);

					i= -2*( vn[0]*view[0]+
					    vn[1]*(view[1]+O.dyview)+ vn[2]*view[2] );

					O.dyref[0]= R.ref[0]- (view[0]+ i*vn[0]);
					O.dyref[1]= R.ref[1]- (view[1]+O.dyview+i*vn[1]);
					O.dyref[2]= R.ref[2]- (view[2]+ i*vn[2]);
				}
			}
		}
	...
	if(ma->mode & MA_SHLESS) {
		...
		if(usegamtab) {
			...
			a= 65535*ir;
			if(a<0) shortcol[3]= 0; else if(a>65535) shortcol[3]= 65535; else shortcol[3]= a;
			a= 65535*ig;
	...
	if(usegamtab) {
		...
		else if(a<0) shortcol[3]= 0;
		else shortcol[3]= a;

		a= 65535.0*(ma->g*ig +ma->ambg +isg);
		if(a>65535)  shortcol[2]= 65535; 
		else if(a<0) shortcol[2]= 0;
		else shortcol[2]= a;

		a= 65535*(ma->b*ib +ma->ambb +isb);
		if(a>65535)  shortcol[1]= 65535; 
		else if(a<0) shortcol[1]= 0;
		else shortcol[1]= a;
  ...
  /* x,y: windowcoordinaat van 0 tot rectx,y */
{
	...
	if(vlaknr<=0) {	/* sky */
		...
		if(R.matren->alpha!=1.0 || alpha!=0.0) {
...
void addps(int *rd, int vlak, ulong z, short ronde)
{
	static PixStr *prev;
	PixStr *ps, *last;
	int vlakand;
...
void add_filt_mask(ulong mask, ushort *col, ulong *rb1, ulong *rb2, ulong *rb3)
{
...
void zbufshadeDA()	/* Delta Accum Pixel Struct */
{
	...
	PixStr *ps;
	float xd, yd, xs, ys;
	ulong coladr, *rz, *rp, *rt, mask, fullmask;
	...
	R.rectdaps= callocN(4*R.rectx*R.recty+4,"zbufDArectd");
	...
	for(y=0;y<=R.recty;y++) {
		...
		if(y>0) {
			...
			/* LET OP: LITTLE ENDIAN */
			
			for(x=0; x<R.rectx; x++,colrt+=4) {
				#ifdef __sgi 
					colrt[0]= ( (char *) (gamtab+colrb[0]) )[0];
					colrt[1]= ( (char *) (gamtab+colrb[2]) )[0];
					colrt[2]= ( (char *) (gamtab+colrb[4]) )[0];
					colrt[3]= ( (char *) (gamtab+colrb[6]) )[0];
				#else
					colrt[0]= ( (char *) (gamtab+colrb[1]) )[1];
					colrt[1]= ( (char *) (gamtab+colrb[3]) )[1];
					colrt[2]= ( (char *) (gamtab+colrb[5]) )[1];
					colrt[3]= ( (char *) (gamtab+colrb[7]) )[1];
				#endif
				colrb+= 8;
			}
			if(R.flag & R_ZTRA) {
			...
			if(R.flag & R_HALO) {
				...
				scanlinehaloPS(rz-R.rectx, rd-2*R.rectx, ((ulong *)colrt)-R.rectx, y-1);
...
void zbufshade()
{
	...
	for(y=0; y<R.recty; y++) {
		...
		if(R.flag & R_HALO) {
			scanlinehalo(rz, (char *)(rp-R.rectx), y);
			rz+= R.rectx;
		}
