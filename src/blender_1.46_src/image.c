...
void ima_ibuf_is_nul(Tex *tex)
{
	...
	if(tex->imaflag & TEX_ANIM5) {
		...
		if(ima->anim==0) ima->anim = openanim(str, IB_rect | IB_cmap);
		if(ima->anim) {
	...
	if(ima->ibuf) {
		...
		if(ima->ibuf->cmap) {
			...
			if(tex->imaflag & TEX_MORKPATCH) {
				...
				if(ima->ibuf->maxcol > 4) {
					ima->ibuf->cmap[2] = 0x80000000;
				}
			}
			
			if(tex->imaflag & TEX_ANIM5) applycmap(ima->ibuf);
		...
		if(R.osa) {
			...
			else if(tex->imaflag & TEX_ANTIALI) {
				antialias(ima->ibuf);
			}
		}
...
int imagewrap(Tex *tex, float *texvec)
{
	...
	if (ima->ok) {
		...
		if(tex->nor) {
			...
			val3= tex->norfac*(val1-val3);
			val2= tex->norfac*(val1-val2);
		...
		if(Talpha) Ta= Tin= ((float)rect[0])/255.0;
