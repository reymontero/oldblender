...
void draw_icon_oops(short x, short y, short type)
{
	ulong *rect;
	extern struct ButIcon BGicon[];
	
	if(x<0 || x>=curarea->winx) return;
	if(y<0 || y>=curarea->winy) return;
...
ulong give_oops_color(short type, short sel, ulong *border)
{
	ulong body;
...
void draw_oops(Oops *oops)
{
	...
	if(oops->id->lib) {
		...
		sboxf(x2-0.2*OOPSX, y2-0.2*OOPSX, x2-0.1*OOPSX, y2-0.1*OOPSX);
	}
	...
	if(str[1] && oopscalex>1.0) {
		ipoco_to_areaco_noclip(v1, mval);
		draw_icon_oops(mval[0], mval[1]-3, oops->type);
	}
	...
	while(ol) {
		...
		cpack(body);
		sboxf(f1-.2, f2-.2, f1+.2, f2+.2);
		cpack(border);
		sbox(f1-.2, f2-.2, f1+.2, f2+.2);
		
		ol= ol->next;
	}

	if(oops->flag & OOPS_REFER) {
		translate(oops->x, oops->y, 0.0);
		cpack(0x0);
		bgnpolygon();
		v1[0]= 0.45*OOPSX; v1[1]= 0.0; v2f(v1);
		v1[0]= 0.46*OOPSX; v1[1]= 0.025*OOPSX; v2f(v1);
		v1[0]= 0.475*OOPSX; v1[1]= 0.04*OOPSX; v2f(v1);
		v1[0]= 0.5*OOPSX; v1[1]= 0.05*OOPSX; v2f(v1);
		v1[0]= 0.525*OOPSX; v1[1]= 0.04*OOPSX; v2f(v1);
		v1[0]= 0.54*OOPSX; v1[1]= 0.025*OOPSX; v2f(v1);
		v1[0]= 0.55*OOPSX; v1[1]= 0.0; v2f(v1);
		endpolygon();
		translate(-oops->x, -oops->y, 0.0);
	}
}
