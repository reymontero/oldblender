...
void do_mesh_key(Mesh *me)
{
	...
	if(me->key->slurph) {
		...
		for(a=0; a<me->totvert; a+=step, cfra+= delta) {
			...
			ctime= system_time(0, 0, cfra, 0.0);
		...
		ctime= system_time(0, 0, (float)CFRA, 0.0);
		if(calc_ipo_spec(me->key->ipo, KEY_SPEED, &ctime)==0) {
			ctime /= 100.0;
			CLAMP(ctime, 0.0, 1.0);
		}
...
void cp_cu_key(Curve *cu, KeyBlock *kb)
{
	Nurb *nu;
	int a, step, tot;
	char *poin;
	...
	while(nu) {
		if(nu->bp) {
			...
			cp_key(a, a+step, tot, poin, cu->key, kb, KEY_BPOINT);
		}
		else if(nu->bezt) {
			...
			cp_key(a, a+step, tot, poin, cu->key, kb, KEY_BEZTRIPLE);
