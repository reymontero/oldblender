...
void clear_track()
{
	...
	mode= pupmenu("OK? %t|Clear Track %x1| Keep transform %x2");
...
void clear_object(char mode)
{
	...
	while(base) {
		if TESTBASELIB(base) {
			if(mode=='r') {
				bzero(base->object->rot, 12);
				bzero(base->object->drot, 12);
			}
			else if(mode=='g') {
				bzero(base->object->loc, 12);
				bzero(base->object->dloc, 12);
			}
			else if(mode=='s') {
				bzero(base->object->dsize, 12);
...
void make_parent()
{
	...
	if(G.scene->id.lib) return;
	if(G.obedit) {
		if(G.obedit->type==OB_MESH) make_vertex_mesh_parent();
	...
	if(par->type==OB_IKA) {
		...
		if(mode==1) {
			draw_ika_nrs(par, 0);
			if(button(&limbnr, 0, 99, "Vertex: ")==0) return;
		}
		else if(mode==2) {
			draw_ika_nrs(par, 1);
			if(button(&limbnr, 0, 99, "Limb: ")==0) return;
		}
		else if(mode==3) {
			ika= par->data;
			if(ika->def==0) {
...
void exit_editmode(int freedata)	/* freedata==0 bij render */
{
	...
	makeDispList(ob);	/* obedit moet 0 zijn voor curve-extrude */
...
void special_editmenu()
{
	...
	if(G.obedit==0) {
		...
		nr= pupmenu("Specials%t|Subivide%x1|Fractal Subdivide%x2|Smooth%x3|Remove Doubles%x4|Hide%x5|Reveal%x6|Select swap%x7|Flip Normals %x8");
		...
		/* nr= pupmenu("Specials%t|Subivide%x1|Hide%x5|Reveal%x6|Select swap%x7"); */
		nr= pupmenu("Specials%t|Subivide%x1|Switch Direction");
					...
					if(ob->disp.first) {
						...
						*basen= *base;
						...
						if(ob1->totcol) {
							...
							for(a=0; a<ob1->totcol; a++) id_us_plus(me->mat[a]);
						}
...
void linkmenu()
{
	...
	if(event==1) {
		...
		if(strncmp(strp, "DataBrow", 8)==0) {
			...
			activate_databrowse(G.scene, ID_SCE, 0, B_INFOSCE, link_to_scene );
				...
				if(event==2 || event==5) {  /* obdata */
					...
					if( matarar && obmatarar) {
						...
						*totcolp= ob->totcol;
						...
						/* users ophogen */
						for(a=0; a<obt->totcol; a++) {
							if(obt->mat[a]) id_us_plus(obt->mat[a]);
							if( (*matarar)[a]) id_us_plus((*matarar)[a]);
						}
	...
	*/
	...
	while(base) {
		...
		if( (base->lay & G.vd->lay) && base->object->id.lib==0) {
			...
			tr= ob->track;
			if(tr) if(TESTBASE(tr)) if((base->flag & SELECT)==0)  base->flag |= BA_PARSEL;
			...
			if(ob->type==OB_IKA) {
				Ika *ika= ob->data;
				if(ika->parent && (ika->parent->flag & SELECT)) base->flag |= BA_WHERE_UPDATE;
			}
...
void make_trans_verts(float *min, float *max)
{
	...
	/* Mat4MulVecfl(G.obedit->obmat, centre); */
	/* Mat4MulVecfl(G.obedit->obmat, centroid); */
}
...
void special_aftertrans_update(char mode, int flip)
{
	...
	if(G.obedit) {
		if(G.obedit->type==OB_MBALL) {
			...
			if(G.f & G_DRAWNORMALS) recalc_editnormals();
		}
	}
	else {
		base= FIRSTBASE;
...
void headerprint(char *str)
{
	...
	cpack(0x0);
	cmov2i(20+curarea->headbutofs, 6);
	fmsetfont(G.font);
	fmprstr(str);
...
void transform(int mode)	/* 'g' 'r' 's' 't' of 'w' */
{
	TransOb *tob;
	TransVert *tv;
	...
	int time, fast=0, a, midtog=0, firsttime=1, proj= 0, fout= 0, cameragrab= 0, gridflag;
	short mval[2], mvalmidtog[2], afbreek=0, doit, xn, yn, xc, yc, xo, yo, event=0, val;
	char str[100];
	...
	if(mode=='r' || mode=='s' || mode=='S') if(xc==32000) {
	...
	if(mode=='w' && G.obedit) {
		...
		startomtrekfac= (90*rad*M_PI)/(360.0*(max[0]-centre[0]));
	}
	...
	while(fout==0 && afbreek==0) {
		...
		if(mval[0]!=xo || mval[1]!=yo || firsttime) {
			if(firsttime) {
				...
				oldval[0]= oldval[1]= oldval[2]= 12.1212;
			}
			firsttime= 0;
				...
				if(dvec[0]!=oldval[0] ||dvec[1]!=oldval[1] ||dvec[2]!=oldval[2]) {
					...
					for(a=0; a<tottrans; a++, tob++, tv++) {
						...
						if(transmain) {
							...
							if(tob->flag & TOB_IKA) VecAddf(tob->eff, tob->oldeff, dvecp);
							...
							VecAddf(tv->loc, tv->oldloc, dvecp);
						}
					}
...
void rightmouse_transform()
{
	...
	while(getbutton(RIGHTMOUSE)) {
		sginap(1);
		getmouseco_areawin(mval);
		if(abs(mval[0]-xs)+abs(mval[1]-ys) > 6) {
			transform('g');
			while(getbutton(RIGHTMOUSE)) sginap(1);
			return;
		}
	}
}
...
void make_local()
{
	...
	/* LETOP: de finctie new_id(..) voegt het id blok opnieuw in!!! */
...
void auto_timeoffs()
{
	...
	bs= basesort= mallocN(4*tot,"autotimeoffs");
	base= FIRSTBASE;
	while(base) {
		if(TESTBASELIB(base)) {
			*bs= base;
	...
	qsort(basesort, tot, 4, (void *)vergbaseco);
