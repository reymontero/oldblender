...
void free_library(lib)
Library *lib;
{

}

void free_libblock(ListBase *lb, ID *id)
{
...
void free_libblock_us(ListBase *lb, ID *id)		/* test users */
{
	...
	if(id->us==0) {
		if( GS(id->name)==ID_OB ) free_object_fromscene(id);
		free_libblock(lb, id);
	}
}
...
/* return 1: nieuwe naam gemaakt */
{
	...
	char left[24], leftest[24], in_use[32];
