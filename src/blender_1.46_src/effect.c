...
void set_buildvars(Object *ob, int *start, int *end)
{
	...
	while(bld) {
		if(bld->type==EFF_BUILD) {
			ctime= system_time(ob, 0, (float)CFRA, bld->sfra-1.0);
			if(ctime < 0.0) {
				*end= *start;
			}
...
void make_particle_keys(int depth, int nr, PartEff *paf, Particle *part, float *force, int deform, MTex *mtex)
{
	...
	if(paf->randfac!=0.0) {
		pa->no[0]+= paf->randfac*(rand()/32767.0 -0.5);
		pa->no[1]+= paf->randfac*(rand()/32767.0 -0.5);
		pa->no[2]+= paf->randfac*(rand()/32767.0 -0.5);
	}
		...
		if(rt1!=rt2) {
			...
			for(b=0; b<paf->child[depth]; b++) {
				...
				*pa= *opa;
				...
				if(paf->randlife!=0.0) {
					pa->lifetime*= 1.0+ paf->randlife*(rand()/32767.0 - 0.5);
				}
...
void build_particle_system(Object *ob)
{
	...
	float ofs, fac, prevobmat[4][4], child, sfraont;
	...
	for(a=0; a<totpart; a++, ftime+=dtime) {
		...
		if(cfralast != cur) {
			...
			system_time(ob, ob->parent, (float)CFRA, 0.0);
		...
		if(paf->normfac!=0.0) {
			sp= mvert->no;
				/* transpose ! */
			vec[0]= imat[0][0]*sp[0]+imat[0][1]*sp[1]+imat[0][2]*sp[2];
			vec[1]= imat[1][0]*sp[0]+imat[1][1]*sp[1]+imat[1][2]*sp[2];
			vec[2]= imat[2][0]*sp[0]+imat[2][1]*sp[1]+imat[2][2]*sp[2];		
...
void object_wave(Object *ob)
{
	...
	if(ob->type==OB_MESH) {
		...
		ctime= system_time(ob, 0, (float)CFRA, 0.0);
		first= 1;
