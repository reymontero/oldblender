...
{
	BevList *bl;
	BevPoint *bevp, *bevpfirst, *bevplast;
	Curve *cu;
	Path *path;
	...
	*fp= 0;
	...
	for(a=0; a<=tot; a++) {
		...
		d= a*fac*path->totdist;
		
		/* we zoeken plek 'd' in het array */
		while( (d>= *fp) && fp<=maxdist && bevp<bevplast) {
			fp++;
			bevp++;
		}
		...
		ft[0]= fac1*bevp->x+ fac2*(bevp+1)->x;
		ft[1]= fac1*bevp->y+ fac2*(bevp+1)->y;
		ft[2]= fac1*bevp->z+ fac2*(bevp+1)->z;
		ft[3]= fac1*bevp->alfa+ fac2*(bevp+1)->alfa;
...
int where_on_path(Object *ob, float ctime, float *vec, float *dir)	/* geeft OK terug */
{
	...
	if(s0==s1 || p2==p3) set_four_ipo(1.0-fac, data, KEY_CARDINAL);
...
void vertex_duplilist(Scene *sce, Object *par)
{
	...
	MVert *mvert;
	Mesh *me;
	float vec[3], pvec[3], dvec[3], qone[4], pmat[4][4], mat[3][3];
	float *q2;
	int lay, totvert, a;
		...
		if(base->object->type>0 && (base->lay & lay) && G.obedit!=base->object) {
			...
			while(ob) {
				if(ob==par) {
					...
					/* if(par->ef==4) totvert= build_schiphol(par, ob->vv->vert); */
...
void particle_duplilist(Scene *sce, Object *par, PartEff *paf)
{
	...
	ctime= system_time(par, 0, (float)CFRA, 0.0);
