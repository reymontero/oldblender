...
EditVlak *evl;
{
	...
	*/
	...
	olded1= (EditVert **)mallocN(8+8*edcount,"addisfaces1");
	olded2= (EditVert **)mallocN(8+8*edcount,"addisfaces2");
	olded3= (EditVert **)mallocN(8+8*edcount,"addisfaces3");
	olded1[0]= evl->v1;
	olded1[1]= evl->v2;
	olded2[0]= evl->v2;
	olded2[1]= evl->v3;
...
void intersect_mesh()
{
	...
	waitcursor(1);
	eld=evlist= (EditVlak **)mallocN(totvlak*4,"intersectVV");
	evl= G.edvl.first;
	while(evl) {
		if(evl->f==1) {
			*(eld++)= evl;
