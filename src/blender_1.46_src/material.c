...
void make_local_material(Material *ma)
{
	...
	if(ma->id.us==1) {
		...
		new_id(0, ma, 0);
		for(a=0; a<8; a++) {
			if(ma->mtex[a]) id_lib_extern(ma->mtex[a]->tex);
		}
	...
	while(ob) {
		...
		if(ob->data==id || sectrue) {
			...
			if(totcol==0) {
				...
				newmatar= callocN(4*totcol, "newmatar");
				if(ob->totcol) {
					memcpy(newmatar, ob->mat, 4*ob->totcol);
					freeN(ob->mat);
				}
				ob->mat= newmatar;
			}
...
void assign_material(Object *ob, Material *ma, int act)
{
	...
	if( act > *totcolp) {
		matar= callocN(4*act, "matarray1");
		if( *totcolp) {
			memcpy(matar, *matarar, 4*( *totcolp ));
			freeN(*matarar);
		}
		*matarar= matar;
		*totcolp= act;
	...
	if(act > ob->totcol) {
		matar= callocN(4*act, "matarray2");
		if( ob->totcol) {
			memcpy(matar, ob->mat, 4*( ob->totcol ));
			freeN(ob->mat);
		}
		ob->mat= matar;
		ob->totcol= act;
...
void init_render_material(Material *ma)
{
	...
	for(a=0; a<8; a++) {
		...
		if(mtex && mtex->tex) {
			...
			ma->mapto |= mtex->mapto;
			if(R.osa) {
				if(mtex->tex->type==TEX_IMAGE) ma->texco |= TEXCO_OSA;
				else if(mtex->tex->type==TEX_PLUGIN) ma->texco |= TEXCO_OSA;
			}
...
void automatname(Material *ma)
{
	...
	b= 4.99*(ref*ma->b);
	nr= r + 5*g + 25*b;
	new_id(&G.main->mat, ma, colname_array[nr]);
