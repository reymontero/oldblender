...
Sector *find_sector_lay(float *vec, float *local, ulong lay)			/* algemene find */
{
	Base *base;
	...
	while(bestcapo==0) {
		...
		for(a=0; a<G.cursector->totcam; a++, capo++) {
			...
			if(capo->visi && (capo->ob->flag & 1)) {
				if(getbutton(LEFTSHIFTKEY)) PRINT4(d, d, d, d, round, capo->visi, capo->dist, capo->d_axis);
			}
		}
...
void evaluate_camera_network(Object *set, short mode)
{
	...
	/* if(getbutton(LEFTSHIFTKEY)) PRINT2(x, d, G.cursector, G.cursector->cfra); */
