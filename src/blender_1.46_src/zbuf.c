...
float *vec1, *vec2;
{
	...
	if(fabs(dx) > fabs(dy)) {
		...
		rectz= (ulong *)(Arectz+R.rectx*(oldy-Aminy) +start);
		...
		rectz= (ulong *)( Arectz+ (start-Aminy)*R.rectx+ oldx );
		...
		if(ap->p[0]) {
			...
			if(totvlak==1) {
				...
				if(totvlak==2) {
					...
					qsort(zrow, totvlak, 12, (void *)vergzvlak);
				}
				...
				while(totvlak>0) {
					...
					if( (R.r.mode & R_OSA ) && a<R.osa) {
						if(totvlak>0) {
							...
							tempcol[0]= longcol[0]/R.osa;
							tempcol[1]= longcol[1]/R.osa;
							tempcol[2]= longcol[2]/R.osa;
							tempcol[3]= longcol[3]/R.osa;
