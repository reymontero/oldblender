...
Life *copy_life(Life *lf)
{
	...
	id_us_plus(lfn->dynamesh);
	id_us_plus(lfn->texmesh);
...
pIpo *make_ob_pipo(Object *ob)
{
	...
	pipo= callocN( sizeof(pIpo) + nr_elems*elemsize*4 + nr_keys*2, "pipo");
	pipo->type= type;
...
void draw_gamedebug_info()
{
	...
	while(ad) {
		...
		if(ad->timer==0) {
			...
			a++;
			cmov2s(curarea->winx-150, curarea->winy-14*a);
	...
	cmov2s(curarea->winx/2 -40, curarea->winy-14);
