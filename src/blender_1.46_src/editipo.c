...
void editipo_changed(int doredraw)
{
	...
	if(doredraw) {
		...
		else if(G.sipo->blocktype==ID_KE) do_spec_key(G.sipo->from);
...
ulong ipo_rainbow(int cur, int tot)
{
	float fac, dfac, sat;
...
EditIpo *select_proj_ipo(rctf *rectf, int event)
{
	EditIpo *ei;
	int a, b;
	short code, hits, buffer[MAXPICKBUF], mval[2];
	...
	if(rectf==0) {
		getmouseco_areawin(mval);
		xmin= (mval[0]-6-G.v2d->mask.xmin)/(float)(G.v2d->mask.xmax-G.v2d->mask.xmin);
		xmax= (mval[0]+6-G.v2d->mask.xmin)/(float)(G.v2d->mask.xmax-G.v2d->mask.xmin);
		ymin= (mval[1]-6-G.v2d->mask.ymin)/(float)(G.v2d->mask.ymax-G.v2d->mask.ymin);
		ymax= (mval[1]+6-G.v2d->mask.ymin)/(float)(G.v2d->mask.ymax-G.v2d->mask.ymin);
		
		xmin= G.v2d->cur.xmin+ xmin*(G.v2d->cur.xmax-G.v2d->cur.xmin);
		xmax= G.v2d->cur.xmin+ xmax*(G.v2d->cur.xmax-G.v2d->cur.xmin);
		ymin= G.v2d->cur.ymin+ ymin*(G.v2d->cur.ymax-G.v2d->cur.ymin);
		ymax= G.v2d->cur.ymin+ ymax*(G.v2d->cur.ymax-G.v2d->cur.ymin);
		...
		if ISPOIN(ei, flag & IPO_VISIBLE, icu) {
			...
			if(ok) {
				...
				if(tot) {
					...
					insertvals= mallocN(4*2*tot, "insertkey_editipo");
...
void add_to_ipokey(ListBase *lb, BezTriple *bezt, int nr, int len)
{
	...
	ikn->data= callocN(4*len, "add_to_ipokey");
	ikn->data[nr]= bezt;
...
void transform_ipo(int mode)
{
	...
	short mval[2], event, val, xo, yo, xn, yn, xc, yc;
	char str[32];
...
void clever_numbuts_ipo()
{
	...
	else far= MAX2(G.vd->far, 10000.0);
	...
	if(totipo_vertsel) {
		...
		if(bezt->f2 & 1) {
			...
			add_numbut(0, NUM|FLO, "LocX:", -1000, 10000, bezt->vec[1]);
			if(totbut==2) add_numbut(1, NUM|FLO, "LocY:", -far, far, bezt->vec[1]+1);
			...
			add_numbut(0, NUM|FLO, "LocX:", -1000, 10000, bezt->vec[0]);
			if(totbut==2) add_numbut(1, NUM|FLO, "LocY:", -far, far, bezt->vec[0]+1);
		
			do_clever_numbuts("Active HandlePoint", totbut, REDRAW);
		}
		else if(bezt->f3 & 1) {
			add_numbut(0, NUM|FLO, "LocX:", -1000, 10000, bezt->vec[0]);
			if(totbut==2) add_numbut(1, NUM|FLO, "LocY:", -far, far, bezt->vec[2]+1);
		...
		if(G.sipo->blocktype==ID_KE) {
			...
			if(kb) {
				add_numbut(0, NUM|FLO, "Pos:", -100, 100, &kb->pos);
...
void ipo_record()
{
	...
	short anim, event, val, xn, yn, mvalo[2], mval[2];
	char str[64];
	...
	data1= callocN(4*(EFRA-SFRA+1), "data1");
	data2= callocN(4*(EFRA-SFRA+1), "data2");
	...
	while(afbreek==0) {
		...
		if(mval[0]!= mvalo[0] || mval[1]!=mvalo[1] || firsttime || (G.qual & LR_CTRLKEY)) {
			...
			else sprintf(str, "Mouse Recording. Use CTRL to start");
	...
	ei1->icu->flag &= ~IPO_LOCK;	
	if(ei2) ei2->icu->flag &= ~IPO_LOCK;	
	
	editipo_changed(0);
	do_ipo(G.sipo->ipo);
	waitcursor(0);
	allqueue(REDRAWVIEW3D, 0);
	addqueue(sa->headwin, REDRAW, 1);	/* headerprint */
