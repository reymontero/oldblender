...
void add_sequence(int type)
{
	...
	#ifdef FREE
	event= pupmenu("Add sequence%t|Images%x1|Movie%x102|Scene%x101|Cross%x2|GammaCross%x3|Add%x4|Sub%x5|Mul%x6|AlphaOver%x7|AlphaUnder%x8|AlphaOverDrop%x9");
	#else
	...
	case 101:
		...
		/* nieuwe menu: */
		IDnames_to_pupstring(&str, &G.main->scene, G.scene, &nr);
...
void change_sequence()
{
	...
	if(last_seq->type & SEQ_EFFECT) {
		...
		event= pupmenu("Change Scene%t|Update Update Start and End");
...
void transform_seq(int mode)
{
	...
	int tot=0, a, b, ix, iy, firsttime=1, afbreek=0, midtog= 0, proj;
	short mval[2], event, val, xo, yo, xn, yn;
	char str[32];
...
void clever_numbuts_seq()
{
	...
	if(last_seq->type==SEQ_PLUGIN) {
		...
		add_numbut(0, TEX, "Name:", 0.0, 21.0, last_seq->name+2);
		
		varstr= pis->varstr;
		for(a=0; a<pis->vars; a++, varstr++) {
			add_numbut(a+1, varstr->type, varstr->name, varstr->min, varstr->max, &(pis->data[a]));
		}
		
		if( do_clever_numbuts(pis->pname, pis->vars+1, REDRAW) ) {
			new_stripdata(last_seq);
			allqueue(REDRAWSEQ, 0);
		}
	}
		...
		add_numbut(0, TEX, "Name:", 0.0, 21.0, last_seq->name+2);
		add_numbut(1, TOG|SHO|BIT|4, "FilterY", 0.0, 1.0, &last_seq->flag);
		/* waarschuwing: alleen een enkele bitjes-button mogelijk: er wordt op kopiedata gewerkt! */
		add_numbut(2, NUM|FLO, "Mul", 0.01, 5.0, &last_seq->mul);
