...
void init_latt_deform(Object *oblatt, Object *ob)
{
	...
	fp= latticedata= mallocN(4*3*deformLatt->pntsu*deformLatt->pntsv*deformLatt->pntsw, "latticedata");
...
int object_deform(Object *ob)
{
	...
	if(ob->parent==0) return;

	if(R.flag & R_RENDERING);
	else if(ob->parent->dt < OB_WIRE) return;
	...
	if(ob->parent->type==OB_LATTICE) {
		...
		if(ob->type==OB_MESH) {
			...
			dl->nr= me->totvert;
			dl->verts= fp= mallocN(3*4*me->totvert, "deform1");
