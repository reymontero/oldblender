...
int filetoname= 0;
float pixels_to_ofs;
char otherdir[120], fslog[200]="", *fsmenu=0;
ScrArea *otherarea;
...
int compare_date(entry1,entry2)	
...
int compare_size(entry1,entry2)	
struct direntry *entry1,*entry2;
{
	/* type is gelijk aan stat.st_mode */
...
void test_flags_file(SpaceFile *sfile)
{
	...
	if(sfile->type==FILE_BLENDER || sfile->type==FILE_LOADLIB) testblender= 1;
...
void read_dir(SpaceFile *sfile)
{
	...
	extern long actnum, totnum;	/* storage.c */
	int num, len;
	char wdir[200];
...
{
	short a;
	char string[200], dir[FILE_MAXDIR], file[FILE_MAXFILE];
...
void calc_file_rcts(SpaceFile *sfile)
{
	...
	bar.xmin= scrollrct.xmin+3;
	bar.xmax= scrollrct.xmax-3;
	h= (scrollrct.ymax-scrollrct.ymin)-6;
	bar.ymax= scrollrct.ymax-3- start*h;
...
void draw_filescroll(SpaceFile *sfile)
{
	...
	cpack(0x707070);
	sboxfi(scrollrct.xmin, scrollrct.ymin, scrollrct.xmax, scrollrct.ymax);

	EmbossBox3(scrollrct.xmin-2, scrollrct.ymin-2, scrollrct.xmax+2, scrollrct.ymax+2, 1, 0x404040, 0xA0A0A0);

	
	cpack(0x909090);
	sboxfi(bar.xmin, bar.ymin, bar.xmax, bar.ymax);

	EmbossBox3(bar.xmin-2, bar.ymin-2, bar.xmax+2, bar.ymax+2, filescrollselect, 0x404040, 0xA0A0A0);
...
void printregel(SpaceFile *sfile, struct direntry *files, int x, int y)
{
	...
	if(boxcol) {
		cpack(boxcol);
		sboxfs(x-17, y-4, x+collumwidth-20, y+11);
	}

	if(files->flags & BLENDERFILE) {
		cpack(0xA0A0);
		sboxfs(x-14, y, x-8, y+7);
	}
	...
	if(s) {
		...
		cmov2i(x - fmgetstrwidth(G.font, files->size), y);
		fmprstr(files->size);
	...
	for(num=0; num<sfile->totfile; num++, file++) {
		if(num==act) {
			...
			if(selecting && num>1) {
				...
				if(file->flags == newflag) redraw= 0;
				file->flags= newflag;
			}
			else {
				if(file->flags & HILITE) redraw= 0;
				else file->flags |= HILITE;
			}
		}
		else file->flags &= ~HILITE;
	}
...
void draw_filetext(SpaceFile *sfile)
{
	...
	/* kader */
	cpack(0x686868);
	sboxfi(textrct.xmin, textrct.ymin, textrct.xmax, textrct.ymax);

	EmbossBox3(textrct.xmin-2, textrct.ymin-2, textrct.xmax+2, textrct.ymax+2, 1, 0x303030, 0xC0C0C0);
	
	scrmask(textrct.xmin+curarea->winrct.xmin, textrct.xmax+curarea->winrct.xmin, textrct.ymin+curarea->winrct.ymin, textrct.ymax+curarea->winrct.ymin);
	...
	for(a=1; a<sfile->collums; a++, x+= collumwidth) {
		cpack(0x303030);
		sboxi(x, textrct.ymin, x, textrct.ymax);
		cpack(0xB0B0B0);
		sboxi(x+1, textrct.ymin, x+1, textrct.ymax);
	}
	...
	files= sfile->filelist+sfile->ofs;
	y= textrct.ymax-FILESEL_DY;
	x= 0;
	coll= 1;
	for(a= sfile->ofs; a<sfile->totfile; a++, files++) {
		
		printregel(sfile, files, x, y+3);
		
		y-= FILESEL_DY;
		if(y<textrct.ymin) {
			if(coll<sfile->collums) {
				y= textrct.ymax-FILESEL_DY;
				x+= collumwidth;
				coll++;
			}
	...
	scrmask(curarea->winrct.xmin,curarea->winrct.ymin, curarea->winrct.xmax, curarea->winrct.ymax);
}
...
void drawfilespace()
{
	...
	DefButBlock(naam, curarea->win, G.font, 10, 6, 3);

	SetButLock( sfile->type==FILE_MAIN && sfile->returnfunc);
	...
	DefBut(BUT,		4, "P", scrollrct.xmin, filebuty2, scrollrct.xmax-scrollrct.xmin, 21, 0, 0, 0, 0, 0);
...
{
	SpaceFile *sfile;
	char *str, group[24], name[120], temp[80];
...
void do_filesel_buttons(short event, SpaceFile *sfile)
{
	...
	if (event == 1) {
		...
		if(sfile->type & FILE_UNIX) {
			if (exist(sfile->dir) == 0) {
				if (okee("Makedir")) recurdir(sfile->dir);
				else parent(sfile);
			}
		}
		checkdir(sfile);
...
void make_file_string (char *string,  char *dir,  char *file) {
	...
	/* Make sure string end in one (and only one) slash */
...
void winqreadfilespace(ushort event, short val)
{
	...
	struct tms voidbuf;
	SpaceFile *sfile;
	int act, do_draw= 0, i, test, newtime;
	short qual, mval[2];
	ushort numlen;
	char head[120], tail[120], str[260];
	...
	if(selecting && getbutton(RIGHTMOUSE==0)) selecting= 0;
	...
	if(val) {
		...
		case RIGHTMOUSE:
			...
			break;
		case MOUSEY:
			act= find_active_file(sfile, mval[0], val-curarea->winrct.ymin);
			set_active_file(sfile, act);
		...
		case MKEY:
			...
			if( countselect(sfile) && getotherdir() ) {
					...
					if (okee(str)) {
						...
						for (i = 0; i<sfile->totfile; i++){
							if (sfile->filelist[i].flags & ACTIVE) {
								...
								if(event==BKEY) sprintf(str, "su root -c 'cd %s; tar cf - \"%s\" | (cd %s; tar xf -)'", sfile->dir, sfile->filelist[i].relname, otherdir);
								else if(event==CKEY) sprintf(str, "cp -r \"%s%s\" %s", sfile->dir, sfile->filelist[i].relname, otherdir);
								else if(event==LKEY) sprintf(str, "ln %s%s %s", sfile->dir, sfile->filelist[i].relname ,otherdir);
								else if(event==MKEY) sprintf(str, "mv %s%s %s", sfile->dir, sfile->filelist[i].relname ,otherdir);
						...
						do_draw= 1;
						if(event==BKEY || event==MKEY) freefilelist(sfile);
						reread_other_fs();
					}
				}
			}
		...
		case TKEY:
			...
			if(event==RKEY && sfile->totfile==2) {
				strcpy(str, "rmdir ");
				strcat(str, sfile->dir);
				if (okee(str)) {
					parent(sfile);
					docmd(str);
				}
			}
			else if(countselect(sfile)) {
			
				if(event==TKEY) sprintf(str, "Touch");
				else if(event==RKEY) {
					sprintf(str, "Remove from");
					strcat(str, sfile->dir);
				}
				...
				if (okee(str)) {
					...
					for (i = 0; i <sfile->totfile; i++) {
						if (sfile->filelist[i].flags & ACTIVE) {
							...
							if(event==TKEY) sprintf(str, "touch %s%s", sfile->dir, sfile->filelist[i].relname);
							else if(event==RKEY) {
								if(qual & LR_SHIFTKEY) sprintf(str, "rm -rf %s%s", sfile->dir, sfile->filelist[i].relname);
								else if(S_ISDIR(sfile->filelist[i].type)) sprintf(str, "rmdir \"%s%s\"", sfile->dir, sfile->filelist[i].relname);
								else sprintf(str, "rm -f \"%s%s\"", sfile->dir, sfile->filelist[i].relname);
							}
		...
		case PKEY:
			if(G.qual & LR_SHIFTKEY) {
				sprintf(str, "anim %s%s", sfile->dir, sfile->file);
				system(str);
			}
			else parent(sfile);
			break;
...
void main_to_filelist(SpaceFile *sfile)
{
	...
	if( sfile->dir[0]==0) {
		...
		if(totlib>1) {
			qsort(firstlib, totlib, sizeof(struct direntry), compare);
		}
	}
	...
	for(a=0; a<sfile->totfile; a++) {
		...
		if(filetoname) {
			if( strcmp(sfile->file, sfile->filelist[a].relname)==0) {
				sfile->ofs= a-( sfile->collums*(curarea->winy-FILESELHEAD-20)/(2*FILESEL_DY));
				filetoname= 0;
