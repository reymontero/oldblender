...
MetaBall *copy_mball(MetaBall *mb)
{
	...
	for(a=0; a<mbn->totcol; a++) {
		id_us_plus(mbn->mat[a]);
	}
	mbn->bb= dupallocN(mbn->bb);
...
void accum_mballfaces(int i1, int i2, int i3, int i4)
{
	...
	if(totindex==curindex) {
		totindex+= 256;
		new= mallocN(4*4*totindex, "vertindex");
		
		if(indices) {
			memcpy(new, indices, 4*4*(totindex-256));
			freeN(indices);
		}
		indices= new;
	}
...
testface(i, j, k, old, bit, c1, c2, c3, c4, p)
CUBE *old;
PROCESS *p;
...
void polygonize(PROCESS *mbproc)
{
	...
	for(a=0; a<totelem; a++) {
		...
		out.x= in.x+mainb[a]->rad;
		out.y= in.y+mainb[a]->rad;
		out.z= in.z+mainb[a]->rad;
		calc_mballco(mainb[a], (float *)&out);
		
		converge(&in, &out, -1.0, mbproc->function, &mbproc->start);
	
		/* NEW: zorg voor correcte uitgangspositie */
		i= ffloor(mbproc->start.x/mbproc->size + 0.5);
		j= ffloor(mbproc->start.y/mbproc->size + 0.5);
		k= ffloor(mbproc->start.z/mbproc->size + 0.5);
		...
		if( setcenter(mbproc->centers, i, j, k)==0 ) {
			...
			for (n = 0; n < 8; n++)
				ncube->cube.corners[n] = setcorner(mbproc, i+BIT(n,2), j+BIT(n,1), k+BIT(n,0));
		}
	}
...
float init_meta(Object *ob)	/* return totsize */
{
	...
	for(a=0; a<totelem; a++) {
		/* threshold correctie bij meerdere ballen */
		if(a) {
...
void metaball_polygonize(Object *ob)
{
	...
	mainb= mallocN(4*MB_MAXELEM, "mainb");
	...
	if(curindex) {
		...
		dl->verts= ve= mallocN(4*3*a, "mballverts");
		dl->nors= no= mallocN(4*3*a, "mballnors");
